    ...
    <div class="cat pa-md-4">
        <v-card> 
            <div> 
                <v-card class="d-flex"  flat tile > 
                    <h2  class="ml-5 mr-auto mt-2">Product Management</h2> 
            ...
            <v-card class="pa-md-2 ma-2"> 
                <v-simple-table>
                    ...
                    <tbody>
                        ...
                        <td>{{ item.name }}</td>
                        <td>{{ item.price }}</td>
                        <td v-if=" item.Category == null">{{ "Deleted" }}</td>
                        <td v-else>{{ item.Category.name  }}</td>
                        <td>
        ...
        <v-dialog  max-width="500"  v-model="modal.add" persistent> 
            ...
            <v-card>
                ...
                <v-card-text> 
                    <div class="pt-5">
                        <h3>Category</h3>
                        <v-select :items="cat"  label="Category" solo item-text="name" v-model="form.cat_id" item-value="cat_id"></v-select>
                    </div>
                    <div>
                        <v-text-field label="Product Name" outlined clearable v-model="form.name"></v-text-field>
                    </div> 
                    <div>
                        <v-text-field label="Product Price" outlined clearable v-model="form.price" type="number"></v-text-field>
                    </div> 
                </v-card-text>
                <v-card-actions class="justify-end">
                <v-btn text  @click="modal.add = false">Cancel</v-btn>
                <v-btn  class="pl-7 pr-7"  color="warning" rounded @click="addMenu">
                        Save
                    </v-btn>
                </v-card-actions>
            </v-card>
        ...
        <v-dialog  max-width="500"  v-model="modal.edit" persistent> 
            ...
            <v-card>
                ...
                <v-card-text> 
                    <div class="pt-5">
                        <h3>Category</h3>
                        <v-select :items="cat"  label="Category" solo item-text="name" v-model="selected.cat_id" item-value="cat_id"></v-select>
                    </div>
                    <div>
                        <v-text-field label="Product Name" outlined clearable v-model="selected.name"></v-text-field>
                    </div> 
                    <div>
                        <v-text-field label="Product Price" outlined clearable v-model="selected.price" type="number"></v-text-field>
                    </div> 
                </v-card-text>
                <v-card-actions class="justify-end">
        ...
        <v-dialog v-model="modal.delete" persistent max-width="390">
            ...
            <v-card-actions>
                ...
                <v-btn color="green darken-1" text @click="delete_menu()">
                Agree
                </v-btn>
            </v-card-actions>
            </v-card>
    ...
    methods:{
        ...
        async initialize(){
            try{
                ...
                this.cat = fetch.data;
                let product =  await axios.get(this.ipaddress+'/api/menu');
        ...
        async addMenu(){
            try{
                let fetch =  await axios.post(this.ipaddress+'/api/addMenu', this.form);
        ...
        delete_menu() {
            axios.delete(this.ipaddress + "/api/deleteMenu/" + this.selected.id)
                .then((response) => {
        ...
        _update_Cat(type) {
            ...
            axios.put(this.ipaddress + "/api/editMenu", this.selected)
                .then((response) => {
